...
$sql.=" FROM guru";
$query=mysqli_query($koneksi, $sql) or die("ajax-data-guru.php: get Guru");
...
if( !empty($requestData['search']['value']) ) {
	...
	$query=mysqli_query($koneksi, $sql) or die("ajax-data.php: get Guru");
	$totalFiltered = mysqli_num_rows($query); // when there is a search parameter then we have to modify total number filtered rows as per search result without limit in the query 

	$sql.=" ORDER BY ". $columns[$requestData['order'][0]['column']]."   ".$requestData['order'][0]['dir']."   LIMIT ".$requestData['start']." ,".$requestData['length']."   "; // $requestData['order'][0]['column'] contains colmun index, $requestData['order'][0]['dir'] contains order such as asc/desc , $requestData['start'] contains start row number ,$requestData['length'] contains limit length.
	$query=mysqli_query($koneksi, $sql) or die("ajax-data-guru.php: get Guru"); // again run query with limit
